...
declare -A PACKAGE_CATEGORIES=(
    ...
    ["Utilities"]="konsole evince fzf ripgrep ripgrep-all gnu-netcat multitail"
...
function install_packages() {
    ...
    if ((${#pacman_queue[@]} > 0)); then
        echo -e "\n${BLUE}▸ ${NC}Installing official packages: ${CYAN}${#pacman_queue[@]} queued${NC}"
        ...
        if ((${#retry_via_yay[@]} > 0)); then
            echo -e "${YELLOW}↻ Retrying ${#retry_via_yay[@]} package(s) via AUR helper${NC}"
    ...
    if ((${#aur_queue[@]} > 0)); then
        echo -e "\n${BLUE}▸ ${NC}Installing AUR packages: ${CYAN}${#aur_queue[@]} queued${NC}"
    ...
    echo -e "• ${GREEN}Installed now: $installed_count packages${NC}"
    if ((${#installed_now[@]} > 0)); then
        echo -e "  ${GREEN}- ${NC}${installed_now[*]}"
    fi
    echo -e "• ${YELLOW}Skipped: $skipped_count already installed${NC}"
    if ((${#already_installed[@]} > 0)); then
...
function activate_services() {
    ...
    for svc in "${SERVICES[@]}"; do
        printf "\n${BLUE}▸ ${NC}Configuring service: ${CYAN}%-25s${NC}" "$svc"
    ...
    echo -e "• ${GREEN}Activated: $enabled_count services${NC}"
...
# Main execution flow
main() {
    echo -e "\n${YELLOW}📦 Starting Package Installation Process${NC}"
    echo -e "--------------------------------------------"
